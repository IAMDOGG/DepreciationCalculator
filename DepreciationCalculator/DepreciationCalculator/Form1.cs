...
namespace DepreciationCalculator
{
    public partial class Form1 : Form
    {
        ...
        private void CalculateSLN(double initialCost, double salvageValue, int usefulLife)
        {
            ...
            listBox1.Items.Add(string.Format("{0,-10} {1,-20} {2,-20}", "Year", "Depreciation", "Book Value"));
            for (int year = 1; year <= usefulLife; year++)
            {
                bookValue -= depreciation;
                listBox1.Items.Add(string.Format("{0,-15} {1,-20:C2} {2,-20:C2}", year, depreciation, bookValue));
            }
        }
        ...
        private void btn_UpdateSchedule_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (rd_StraightLine.Checked)
                {
                    CalculateSLN(initialCost, salvageValue, usefulLife);
                }
                else if (rd_SumofYears.Checked)
                {
